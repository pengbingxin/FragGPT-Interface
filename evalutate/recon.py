...
import sys
sys.path.append(r'D:\frag_gpt\1\FFLOM-main\FFLOM-main')
from utils import *
...
parser = argparse.ArgumentParser(description='FragGPT model')
...
for idx in range(len(frag_nodes)):
    ...
    ##  idx   len(smiles)/(idx+1)     len(set(smiles))/max(len(smiles),1)     recon/(idx+1)