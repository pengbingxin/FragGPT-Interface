...
# 使用RDLogger来禁止RDKit警告信息的输出
RDLogger.DisableLog('rdApp.*')
# 使用warnings模块来过滤警告信息
warnings.filterwarnings("ignore")
# 现在您可以在没有RDKit警告输出的情况下运行您的RDKit代码
import pandas as pd
...
with open(args.gen_data, 'r') as f:
    full = [line.strip('\n') for line in f]
data = pd.read_csv(args.gen_data)

frag_mols =  data['fragment'].values.tolist()
in_mols = data['in_mols'].values.tolist()
gen_mols = data['smiles'].values.tolist()
...
if args.linker_design:
    ...
    print(f'unique is  {len(rs_train_canon_unique)/len(rs_train_canon)}')
...
'''
python evalutate/evaluate.py --train_data data/delinker_zinc/data/train.txt --gen_data ./outputs/linker/casf188/casf188.csv --linker_design

python evalutate/evaluate.py --train_data data/delinker_zinc/data/train.txt --gen_data ./outputs/test.csv --linker_design
'''
